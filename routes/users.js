...
usersRouter.put('/:id', (req, res) => {
  ...
  Promise.all([
    User.findOne(req.params.id),
    User.modifyAuthorisation(req.body.token, req.params.id),
    User.findByEmailWithDifferentId(req.body.email, req.params.id),
  ])
    .then(([user, authorisation, otherUserWithEmail]) => {
      existingUser = user;
      req.body.token = calculateToken((req.body.email) ? req.body.email : user.email);
      if (!existingUser) return Promise.reject('RECORD_NOT_FOUND');
      if (!authorisation) return Promise.reject('PERMISSION_NOT_ALLOWED');
    ...
    .catch((err) => {
      ...
      if (err === 'PERMISSION_NOT_ALLOWED')
        return res.status(401).send(`Trying to modify user data without permission.`);
      if (err === 'DUPLICATE_EMAIL')
        res.status(409).json({ message: 'This email is already used' });
      else if (err === 'INVALID_DATA')
        res.status(422).json({ validationErrors });
